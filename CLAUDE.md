...
- **Router manual y automático** (`/model <nombre>`, `/route auto` / `/route manual <nombre>`) — cambia de provider sin tocar el Agent Loop. En modo `auto`, clasifica el texto del turno con reglas deterministas por keyword (nunca ML, según §6) en una de 4 categorías (`vision`/`reasoning`/`cheap`/`coding`), y elige el primer provider de esa categoría que esté registrado *y* declare la capability que la categoría exige (`ModelCapabilities.vision`/`.reasoning`) — si ninguno califica, cae de vuelta al provider manual en vez de fallar el turno.
